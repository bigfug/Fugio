...
// A section of copy and paste id's
...
template <typename T> struct PinEnt
{
...
public:
	...
	template <class T> bool pinInput( const QString &pName, PinEnt<T> &pPinEnt, const QUuid &pControlUUID, const QUuid &pLocalId )
	{
		Q_ASSERT( pPinEnt.mPin.isNull() );

		pPinEnt.mEnt = qobject_cast<T>( mNode->createPin( pName, PIN_INPUT, QUuid::createUuid(), pLocalId, pPinEnt.mPin, pControlUUID ) );

		return( pPinEnt.mEnt != Q_NULLPTR );
	}
	...
	template <class T> bool createProperty( const QString &pName, PinEnt<T> &pPinEnt, const QUuid &pControlUUID, const QUuid &pLocalId )
	{
		Q_ASSERT( pPinEnt.mPin.isNull() );

		pPinEnt.mEnt = qobject_cast<T>( mNode->createProperty( pName, PIN_INPUT, QUuid::createUuid(), pLocalId, pPinEnt.mPin, pControlUUID ) );
	...
	template <class T> bool pinOutput( const QString &pName, PinEnt<T> &pPinEnt, const QUuid &pControlUUID, const QUuid &pLocalId )
	{
		Q_ASSERT( pPinEnt.mPin.isNull() );

		pPinEnt.mEnt = qobject_cast<T *>( mNode->createPin( pName, PIN_OUTPUT, QUuid::createUuid(), pLocalId, pPinEnt.mPin, pControlUUID ) );
