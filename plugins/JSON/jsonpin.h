...
class JsonPin : public fugio::PinControlBase, public fugio::VariantHelper<QJsonDocument>, public fugio::SerialiseInterface
{
	Q_OBJECT
	...
	Q_CLASSINFO( "URL", WIKI_PIN_URL( "Json" ) )
	Q_CLASSINFO( "Contact", "http://www.bigfug.com/contact/" )

public:
	Q_INVOKABLE explicit JsonPin( QSharedPointer<fugio::PinInterface> pPin );

	virtual ~JsonPin( void ) {}
	...
	virtual void deserialise( QDataStream &pDataStream ) Q_DECL_OVERRIDE
	{
		...
		if( pDataStream.commitTransaction() )
		{
			...
			setVariant( 0, QJsonDocument::fromJson( V ) );
		...
		for( QByteArray A : L )
		{
			mValues << QJsonDocument::fromJson( A );
		}
	}
};
