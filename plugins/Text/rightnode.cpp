...
RightNode::RightNode( QSharedPointer<fugio::NodeInterface> pNode )
...
void RightNode::inputsUpdated( qint64 pTimeStamp )
{
	...
	for( fugio::NodeInterface::UuidPair UP : mNode->pairedPins() )
	{
		...
		{
			QString		NewVar = SrcVar.index( i ).toString().right( Index );
...
QUuid RightNode::pairedPinControlUuid( QSharedPointer<fugio::PinInterface> pPin ) const
{
	return( pPin->direction() == PIN_INPUT ? PID_STRING : PID_STRING );
}

QList<QUuid> RightNode::pinAddTypesInput() const
{
...
bool RightNode::canAcceptPin( fugio::PinInterface *pPin ) const
{
